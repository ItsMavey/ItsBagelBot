...
func (_q *UserQuery) QueryConfigs() *ConfigsQuery {
	query := (&ConfigsClient{config: _q.config}).Query()
...
func (_q *UserQuery) QueryTokens() *TokensQuery {
	query := (&TokensClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) QueryTimers() *TimersQuery {
	query := (&TimersClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) WithConfigs(opts ...func(*ConfigsQuery)) *UserQuery {
	query := (&ConfigsClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithTokens(opts ...func(*TokensQuery)) *UserQuery {
	query := (&TokensClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithTimers(opts ...func(*TimersQuery)) *UserQuery {
	query := (&TimersClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withConfigs; query != nil {
		if err := _q.loadConfigs(ctx, query, nodes, nil,
			func(n *User, e *Configs) { n.Edges.Configs = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadConfigs(ctx context.Context, query *ConfigsQuery, nodes []*User, init func(*User), assign func(*User, *Configs)) error {
	...
	query.withFKs = true
	query.Where(predicate.Configs(func(s *sql.Selector) {
...
func (_q *UserQuery) loadTokens(ctx context.Context, query *TokensQuery, nodes []*User, init func(*User), assign func(*User, *Tokens)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Tokens(func(s *sql.Selector) {
...
func (_q *UserQuery) loadTimers(ctx context.Context, query *TimersQuery, nodes []*User, init func(*User), assign func(*User, *Timers)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Timers(func(s *sql.Selector) {
	...
	for _, n := range neighbors {
		fk := n.user_timers
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_timers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
