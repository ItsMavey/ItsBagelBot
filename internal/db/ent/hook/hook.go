...
func (f ConfigsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ConfigsMutation); ok {
		return f(ctx, mv)
	}
...
// function as Timers mutator.
type TimersFunc func(context.Context, *ent.TimersMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TimersFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TimersMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TimersMutation", m)
}
...
func (f TokensFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TokensMutation); ok {
		return f(ctx, mv)
	}
